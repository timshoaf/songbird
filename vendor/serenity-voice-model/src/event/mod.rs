...
pub enum Event {
    ...
    /// DAVE protocol transition preparation.
    DavePrepareTransition(DavePrepareTransition),
    /// DAVE protocol transition execution.
    DaveExecuteTransition(DaveExecuteTransition),
    /// DAVE protocol transition readiness.
    DaveTransitionReady(DaveTransitionReady),
    /// DAVE epoch preparation.
    DavePrepareEpoch(DavePrepareEpoch),
    /// DAVE invalid commit/welcome report.
        ...
        A: MapAccess<'de>,
    {
                ...
                Some("op") => {
                    ...
                    let valid_op = Opcode::deserialize(des).map_err(|_| {
                        DeError::invalid_value(
                            ...
                            &"opcode in known Discord voice opcode set",
                        )
                    })?;
                    op = Some(valid_op);
                },
        ...
        (match op {
            ...
            | Opcode::DaveMlsWelcome => {
                Err(serde_json::Error::custom("binary DAVE MLS opcode is not JSON-decodable"))
            },
